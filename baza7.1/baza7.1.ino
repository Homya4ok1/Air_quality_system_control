...
#define PIN_LED2 33  // CONDSTSONER
#define PIN_LED3 25  // AIR CONTROL
...
String header;
TS_Point p;
int levelTemp = 0;  // змінна значення слайдеру (0 - 255)
...
const int perioddelay = 30;  // значення часу до якого викликається disconnect
...
void rectangle(int x) {
  switch (x) {
    ...
    case 4:  //чорний квадрат для рівня заряду 1 батареї
      ...
      break;
    case 5:
      if (flag1 == 1 && flag2 == 0)  //чорний квадрат для diskonect
      {
        // Fill the other digit with background color:
...
void tochsk() {
  ...
  //batton settings
  if (p.x >= 280 && p.x <= 280 + 20 &&  //height (50), width(70)
      p.y >= 170 && p.y <= 170 + 20 && countbut4 == 0) {
    flag4 = !flag4;
    countbut4 = 12;
  }
  ...
  if (flag4 == 1) {  // обробник натиску кнопки автоматичного режиму
    ...
    int x = map(p.x, 5, 200, TEMPERATURE_MIN, TEMPERATURE_MAX);
    if (x >= TEMPERATURE_MIN && x <= TEMPERATURE_MAX) {  // Перевірка, чи значення в межах діапазону
      temperature = x;
      Serial.print(" x ");
      Serial.println(x);

      drawUI();  // Оновлення відображення на екрані
      qw = 0;
    }
  }
  ...
  // Перевірка наявності дотику
  if (ts.touched()) {  // Перевірка наявності дотику
                       // brightness = 255;
  ...
  if (currentMillis - previousMillis2 >= interval) {  // Перевірка часу
    previousMillis2 = currentMillis;                  // Оновлення значення яскравості
    analogWrite(TFT_LED, 200);
    Serial.print(" rtdthft ");
  }
}
...
void UpdateSlider() {
  ...
  levelTemp = t_state.toInt();  //рівень слайдера
  Serial.print("UpdateSlider");
  Serial.println(levelTemp);
  // now set the PWM duty cycle
  ledcWrite(0, levelTemp);

  airtemp = map(levelTemp, 0, 255, 18, 30);  //18 до 30 C.
  strcpy(buf, "");
...
void automatic() {
  ...
  if (LED1 == 1) {
    ...
    if (((airtemp <= (avrTemp - 2)) && (airtemp >= (avrTemp + 2))) || ((airtemp >= (avrTemp + 2)) && (airtemp <= (avrTemp - 2)))) {  //гестеризис температурного рівня
...
void scrin() {  //графічний дизайн екрану
...
void drawUI() {                   //слайдер
  ...
  int sliderX = map(temperature, TEMPERATURE_MIN, TEMPERATURE_MAX, 10, 230);
  tft.fillRect(10, 50, 230, 10, ILI9341_BLUE);
  tft.fillRect(sliderX - 5, 45, 10, 30, ILI9341_RED);
  Serial.print(" sliderX ");
